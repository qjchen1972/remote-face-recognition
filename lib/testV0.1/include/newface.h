...
extern "C" {
	...
	_declspec(dllexport) int  init_face(const char* init_net, const char*  predict_net);
	_declspec(dllexport) int  init_modify_face(const char* init_net, const char*  predict_net);

//3通道
	_declspec(dllexport) int  get_vec(cv::Mat img,
	         cv::Rect rect, 
	...
	//用于测试，不需要导出
	_declspec(dllexport) void get_modify_face(Mat src, Mat &dst);
//计算cos距离
	_declspec(dllexport) float  cal_cos(std::vector<float> a, std::vector<float> b);

//设置区域内收比率,不设置就是缺省区域
	_declspec(dllexport) void set_face_ext(float top_x, float top_y);
//设置多远距离开始进行内收,缺省是rect.width=64时内收
	_declspec(dllexport) void set_distance_threshold(int width);
