...
public:
	...
	Mat  get_face_with_center(cv::Rect rect, Mat  img, vector<Point2f> five)
	{
		...
		if (abs(angle) > 0.01)
		{
			...
			vector<Point2f> newfive;
			for (int i = 0; i < five.size(); i++)
				newfive.push_back(Point2f(five[i].x - temp_rect.x, five[i].y - temp_rect.y));
	...
	Mat  get_face(cv::Rect rect, Mat  img, vector<Point2f> five)
	{
	...
	Mat  get_face_with_ccl(cv::Rect rect, Mat  img, vector<Point2f> five)
	{		
		//get the direction of eye
		Point eye_direction(five[1].x - five[0].x, five[1].y - five[0].y);
		//calc rotation angle in radians
...
private:
	...
	//Í¼ÏñÔ¤´¦Àí
	void pre_img(cv::Rect rect, Mat  &img, vector<Point2f> &five)
	{
		return;
	...
	void align_face_with_center(Mat &img, float angle, vector<Point2f> five)
	{
		vector<Point2f> src, dest;
